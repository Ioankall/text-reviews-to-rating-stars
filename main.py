...
if __name__ == '__main__':
    ...
    reviews_1 = db.reviews.find({'stars': 1}, {'text': 1, 'stars': 1}).limit(1000)
    reviews_2 = db.reviews.find({'stars': 2}, {'text': 1, 'stars': 1}).limit(1000)
    reviews_3 = db.reviews.find({'stars': 3}, {'text': 1, 'stars': 1}).limit(1000)
    reviews_4 = db.reviews.find({'stars': 4}, {'text': 1, 'stars': 1}).limit(1000)
    reviews_5 = db.reviews.find({'stars': 5}, {'text': 1, 'stars': 1}).limit(1000)
    reviews = chain(reviews_1, reviews_2, reviews_3, reviews_4, reviews_5)

    result, stars = preprocessing.process_data(reviews, lexicon='load')
